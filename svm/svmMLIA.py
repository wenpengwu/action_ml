...
def innerL(i,os):
    ...
    if (os.labels[i]*ei <-os.tol and os.alphas[i]<os.C) or (os.labels[i]*ei>os.tol and os.alphas[i]>0):
        ...
        update_ek(os,i)
        b1 = os.b -ei - os.labels[i]*(os.alphas[i]-alphaIold)*os.X[i,:]*os.X[i,:].T - os.labels[j]*(os.alphas[j]-alphaJold)*os.X[i,:]*os.X[i,:].T
        b2 = os.b -ej-os.labels[i]*(os.alphas[i]-alphaIold)*os.X[i,:]*os.X[i,:].T - os.labels[j]*(os.alphas[j]-alphaJold)*os.X[i,:]*os.X[i,:].T
    ...
    while iter<iter_num and (alpha_pairs_changed>0 or entire_set):
        ...
        if entire_set:
            ...
            for i in non_bound_ids:
                alpha_pairs_changed+=innerL(i,os)
                print('non-bound iterm:%d i:%d ,pair changed %d'%(iter,i,alpha_pairs_changed))
            iter+=1
...
def kernel_trans(X,A,kTup):
    ...
    else:raise NameError('休斯顿我们有个问题--核方法无法识别')
    return K
...
def kernel_innerL(os,i):
    ...
    if (os.labels[i] * ei < -os.tol and os.alphas[i] < os.C) or (os.labels[i] * ei > os.tol and os.alphas[i] > 0):
        ...
        os.alphas[i] += os.labels[j] * os.labels[i] * (alphaJold - alphaIold)
        update_ek(os, i)
        b1 = os.b - ei - os.labels[i] * (os.alphas[i] - alphaIold) * os.X[i, :] * os.X[i, :].T - os.labels[j] * (
                    os.alphas[j] - alphaJold) * os.X[i, :] * os.X[i, :].T
        b2 = os.b - ej - os.labels[i] * (os.alphas[i] - alphaIold) * os.X[i, :] * os.X[i, :].T - os.labels[j] * (
                    os.alphas[j] - alphaJold) * os.X[i, :] * os.X[i, :].T
